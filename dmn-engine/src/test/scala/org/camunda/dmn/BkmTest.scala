...
class BkmTest extends FlatSpec with Matchers with DecisionTest {
  ...
  {
    eval(literalExpression, "literalExpression", Map("x" -> 2, "y" -> 3)) should be(Result(5)) 
  }
  
  "A BKM with a Context" should "be invoked as function" in
  {
    eval(context, "context", Map("x" -> 2, "y" -> 3)) should be(Result(Map(
      "Sum" -> 5,
      "Multiply" -> 6
    ))) 
  }
  ...
  {
    eval(relation, "relation", Map("x" -> 2, "y" -> 3)) should be(Result(List(
        Map("rate" -> "A", "fee" -> 5),
        Map("rate" -> "B", "fee" -> 7.5),
        Map("rate" -> "C", "fee" -> 8.75)
    ))) 
  ...
  {
    eval(decisionTable, "decisionTable", Map("x" -> "Business", "y" -> 7)) should be(Result(0.1)) 
  }
