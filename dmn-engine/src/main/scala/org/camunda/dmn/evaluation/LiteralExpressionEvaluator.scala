...
import org.camunda.feel.interpreter.{ RootContext, ValFunction, DefaultContext, Val }
...
class LiteralExpressionEvaluator(feelEngine: FeelEngine) {
  ...
  def evalExpression(literalExpression: ParsedLiteralExpression, context: EvalContext): Either[Failure, Val] = {
  ...
  def evalExpression(expression: ParsedExpression, context: EvalContext): Either[Failure, Val] = {
    ...
    feelEngine.eval(expression, evalContext) match {
      ...
      case EvalValue(value)      => Left(Failure(s"expected value but found '$value'"))
