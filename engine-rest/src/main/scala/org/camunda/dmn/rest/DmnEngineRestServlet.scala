...
class DmnEngineRestServlet(engine: StandaloneEngine) extends ScalatraServlet with JacksonJsonSupport with FileUploadSupport {
  ...
  case class DeploymentResult(deployedDecisions: Iterable[Decision], failures: Iterable[Failure])
  ...
  post("/decisions") 
  {
    val result = fileParams.map{ case (name: String, file: FileItem) => 
    {
      engine
  ...
  private def decisionDto(d: DeployedDecision) = Decision(d.decisionId, d.decisionName, d.resource)
